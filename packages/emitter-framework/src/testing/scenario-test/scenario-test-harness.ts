...
export function executeScenarios(
  ...
  describe.only("Scenarios", function () {
    ...
    if (hasOnlyScenarios) {
      describeScenarios(SCENARIOS_LOCATION, testLibrary, emitterOutputDir, snippetExtractor, false);
    } else {
      describeScenarios(SCENARIOS_LOCATION, testLibrary, emitterOutputDir, snippetExtractor, true);
    }
  });
}
