 ...
 */
function emitRawServer(ctx: HttpContext, operationsModule: Module): Module {
  const serverRawModule = createModule("server_raw", operationsModule);
  ...
  if (operation.parameters.body) {
    ...
    switch (contentType) {
      ...
      case "multipart/form-data":
        ...
        yield `  if (!boundary) {`;
        yield `      reject("Invalid request: missing boundary in content-type.");`;
        yield `  }`;
        yield "";
        yield `  const chunks = [];`;
        yield `  request.on("data", function appendChunk(chunk) { chunks.push(chunk); });`;
        ...
        yield `      const headers = Object.fromEntries(headerText.split("\\r\\n").map((line) => line.split(": ", 2)));`;
...
function* emitResultProcessingForType(ctx: HttpContext, target: Type): Iterable<string> {
  ...
  for (const property of target.properties.values()) {
    if (isHeader(ctx.program, property)) {
      ...
      if (!body) yield `delete result.${parseCase(property.name).camelCase};`;
